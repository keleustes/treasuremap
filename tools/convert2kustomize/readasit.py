        ...
        if (thepattern):
            if thekey not in node:
                if (thepattern == "DB_NAME") and (thekey == "path"):
                    node[thekey] = "/" + varname
                elif (thepattern == "VHOST_NAME") and (thekey == "path"):
                    node[thekey] = "/" + varname
                else:
...
def add_target_state(filename):
    ...
    with open(filename, 'r') as stream:
        ...
        docs2 = list(docs)
        for doc in docs2:
            if (doc["kind"] == "ArmadaChart") or (doc["kind"] == "ArmadaChartGroup") or (doc["kind"] == "ArmadaManifest"):
                doc["spec"]["target_state"] = "uninitialized"
